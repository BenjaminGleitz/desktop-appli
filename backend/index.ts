...
`);
    ...
    app.get('/conversations', async (req, res) => {
        try {
            const [rows] = await db.execute<RowDataPacket[]>('SELECT id, title FROM conversation');
            res.json(rows);
        } catch (error) {
            res.status(500).json({error: 'Database error'});
        }
    ...
    app.post('/create-conversation', async (req, res) => {
        ...
        try {
            const [result] = await db.execute<ResultSetHeader>('INSERT INTO conversation (title) VALUES (?)', [title]);
